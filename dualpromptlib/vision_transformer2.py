...
from dualpromptlib.prompt2 import EPrompt
from dualpromptlib.attention import PreT_Attention
...
class VisionTransformer2(nn.Module):
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        # print("check vit head 2")
        # print(x.shape)

        res['pre_logits'] = x

        # print("check vit head 3")
        # print(x.shape)

        x = self.fc_norm(x)
        
        # print("check vit head 4")
        # print(x.shape)

        res['logits'] = self.head(x)
        
        # print("check vit head 5")
        # print(res['logits'].shape)

        return res


    def forward_head_with_proto(self, res, proto, pre_logits: bool = False):
        x = res['x']
        # print("check vit head ")
        # print(x.shape)
    ...
    def forward_with_proto(self, x, task_id=-1, cls_features=None, proto=None, train=False):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_head_with_proto(res,proto)
        return res


    def forward_get_prelogits(self, x, task_id=-1, cls_features=None, train=False):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_head_prelogits(res)
        return res


    def forward_head_prelogits(self, res, pre_logits: bool = False):
        x = res['x']
        # print("check vit head ")
        # print(x.shape)
        ...
        pre_logits_x = self.fc_norm(x)
        
        # print("check pre_logits_x shape")
        # print(pre_logits_x.shape)
...
@torch.no_grad()
def _load_weights(model: VisionTransformer2, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        VisionTransformer2, variant, pretrained,
