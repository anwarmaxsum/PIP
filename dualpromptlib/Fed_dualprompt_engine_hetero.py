...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if iter == 0:
            cl_list =list(set(target.tolist())) 
            print(cl_list)
            # print(input.shape)
            tid = int(math.floor(min(cl_list)/len(cl_list)))
            # print("tid: "+str(tid))
        ...
        output = model(input, task_id=tid, cls_features=cls_features, train=set_training_mode)
        # output = model(input, task_id=-1, cls_features=cls_features, train=set_training_mode)
...
def train_pertask_full_v2(models, models_without_ddp, original_models, 
                    criterion, data_loaders, optimizers, lr_schedulers, device: torch.device, 
                    class_masks=None, task_id=0, list_task_ids=[], args = None,):
    ...
    for epoch in range(args.epochs):
        for n in range(len(models)):            
            train_stats = train_one_epoch(model=models[n], original_model=original_models[n], criterion=criterion, 
                                        data_loader=data_loaders[n][task_id]['train'], optimizer=optimizers[n], 
                                        device=device, epoch=epoch, max_norm=args.clip_grad, 
                                        set_training_mode=True, task_id=list_task_ids[n], class_mask=class_masks[n], args=args,)
...
def evaluate_all_clients(models, models_without_ddp, original_models, 
    ...
    acc_all_models=[]
    for n in range(len(models)):  
        test_stats, avg_stat = evaluate_till_now(model=models[n], original_model=original_models[n], data_loader=data_loaders[n], device=device, 
                                    task_id=task_id, class_mask=class_masks[n], acc_matrix=acc_matrix, args=args)
        acc_all_models.append(avg_stat[0])
