...
class EPrompt(nn.Module):
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            if self.batchwise_prompt:
                ...
                try:
                    if prompt_id.shape[0] < self.pool_size:
                        ...
                        # prompt_id = torch.cat([prompt_id, torch.full((self.pool_size - prompt_id.shape[0],), torch.min(idx.flatten().flatten()), device=prompt_id.device)])
                ...
                idx = major_prompt_id.expand(x_embed_norm.shape[0], -1).contiguous() # B, top_k
            
            
            # print("check e_rompt 2.1")
            # print(prompt_key_norm.shape)
            # print(idx.shape)

            if prompt_mask is not None:
                idx = prompt_mask # B, top_k
                if(idx.shape[0] < x_embed_norm.shape[0]):
                    diff = x_embed_norm.shape[0] - idx.shape[0]
                    idx = torch.cat([idx,idx[0:diff]],dim=0)

            # print("check e_rompt 2.15")
            # # print(prompt_key_norm.shape)
            # print(idx.shape)
            # print(idx[:,0])
            ...
            # print("check e_rompt 2.2")
            # print(prompt_key_norm.shape)
            # print(idx.shape)

            batched_key_norm = prompt_key_norm[idx] # B, top_k, C
            # print("check e_rompt 2.5")
